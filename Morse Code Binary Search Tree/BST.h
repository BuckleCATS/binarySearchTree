...
public:
	...
	//Node functions
	void insertNode(BSTNode<B>* current_ptr, char character, std::string code);
	void print(BSTNode<B>* tree); //made this function w/ help from Andy's in-class code of inorderTraversal function
	void search(BSTNode<B>* tree);
	void match(BSTNode<B>* tree, char message);
...
void BST<B>::insertNode(BSTNode<B>* current_ptr, char character, std::string code)
{
	if (isEmpty() == true)
	{
		...
		root = new BSTNode<std::string>(code, character);
	}
	else
	{
